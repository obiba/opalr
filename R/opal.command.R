...
opal.commands <- function(opal, df=TRUE) {
  ...
  res <- opal.get(opal, opal$context, "session", opal.session(opal), "commands")
  if (!df) {
    return(res)
  }
  n <- length(res)
...
opal.command <- function(opal, id, wait=FALSE) {
  ...
  opal.get(opal, opal$context, "session", opal.session(opal), "command", id, query=query)
}
...
opal.command_rm <- function(opal, id) {
  ...
  ignore <- tryCatch(opal.delete(opal, opal$context, "session", opal.session(opal), "command", id), error=function(e){})
}
...
opal.command_result <- function(opal, id, wait=FALSE) {
  ...
  opal.get(opal, opal$context, "session", opal.session(opal), "command", id, "result", acceptType = "application/octet-stream")
}
